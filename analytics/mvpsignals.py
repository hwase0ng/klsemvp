...
def extractSignals(counter, sdict, xpn):
    ...
    def evalVol(sval, mia):
        ...
        def evalLowV():
                    ...
                    if isprev3topP():
                        if plistP[-1] < plistP[-2]:
                            ...
                            if nlistP[-1] >= 0:
                                ...
                                sig, state = -sval, 15
            # elif bottomM:
                elif isprev3topP() and plistP[-1] < plistP[-2]:
                    if bottomP:
                        # 2015-08-05 DUFU
                        sig, state = sval, -30
                    else:
            ...
            if topP:
                ...
                sig, state = evalPrevtopM(sval + 4)
            elif prevbottomM:
                sig, state = evalPrevbottomM(sval + 5)
            else:
                if max(plistM[-3:]) >= 10:
                    if min(nlistM[-3:]) >= 5:
            ...
            def evalP2Max(sval):
                ...
                if newlowP:
                    if topP:
                        if prevbottomM:
                            if nlistM[-1] >= 5:
                                if max(plistM[-3:]) < 10 or max(nlistP[-3:]) <= 0:
                                    # 2019-01-18 ARANK
                                    sig, state = -sval, 1
                                else:
    ...
    if plistM is None or plistP is None or nlistM is None or nlistP is None:
        nosignal = True
    elif (len(nlistP) < 2 or len(plistP) < 2) and busdays < 345:
