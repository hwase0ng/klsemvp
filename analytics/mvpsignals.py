...
def scanSignals(mpvdir, dbg, counter, sdict, pid=0):
    ...
    def printsignal(workdir, trxndate):
        ...
        if not pid:
            workdir = os.path.join(S.DATA_DIR, S.MVP_DIR)
        # outfile = workdir + "signals/" + counter + "-signals." + postfix
        outfile = os.path.join(workdir, "signals", counter + "-signals." + postfix)
        ...
        if not dbg:
            ...
            sss = os.path.join(workdir, "signals", label + "." + trxndate + ".csv")
...
def extractSignals(counter, sdict, xpn):
    ...
    def isprev3topM():
        if mvalley:
            if plenM > 2 and max(plistM[-3:-1]) == max(plistM):
                return True
        else:
            if plenM > 2 and max(plistM[-3:-1]) == max(plistM[:-1]):
                return True
        return False

    def isprev3topP():
        if pvalley:
            if plenP > 2 and max(plistP[-3:-1]) == max(plistP):
                return True
        else:
            if plenP > 2 and max(plistP[-3:-1]) == max(plistP[:-1]):
                return True
        return False

    def isprev3topV():
        if vvalley:
            if plenV > 2 and max(plistV[-3:-1]) == max(plistV):
                return True
        else:
            if plenV > 2 and max(plistV[-3:-1]) == max(plistV[:-1]):
                return True
        return False

    def isprev3bottomM():
        if mpeak:
            if nlenM > 2 and min(nlistM[-3:-1]) == min(nlistM):
                return True
        else:
            if nlenM > 2 and min(nlistM[-3:-1]) == min(nlistM[:-1]):
                return True
        return False

    def isprev3bottomP():
        if ppeak:
            if nlenP > 2 and min(nlistP[-3:-1]) == min(nlistP):
                return True
        else:
            if nlenP > 2 and min(nlistP[-3:-1]) == min(nlistP[:-1]):
                return True
        return False

    def isprev3bottomV():
        if vpeak:
            if nlenV > 2 and min(nlistV[-3:-1]) == min(nlistV):
                return True
        else:
            if nlenV > 2 and min(nlistV[-3:-1]) == min(nlistV[:-1]):
                return True
        return False
    ...
    def evalExtremes(sval):
        ...
        if not sig:
            if newlowM and newlowP and newlowV:
                if topP and topV:
                    if isprev3bottomM():
                        if max(plistM[-3:]) >= 10 or nlistP[-1] < 0:
                            if plistM[-1] >= 10:
                                # 2010-04-05 PADINI
                                sig, state = -sval, 2
                            else:
                ...
                if newlowV:
                    if max(nlistM[-3:]) < 5:
                        ...
                        if nlistM[-1] == min(nlistM[-3:]) or \
                                isprev3bottomP():
                            if prevbottomM and prevbottomP:
                                # 2018-07-02 PADINI
                                sig, state = -sval, 5
                            else:
                                # 2014-04-02 KESM
        ...
        if not sig:
            if tripleM in p3d and tripleP in p3d and tripleV in p3d:
                if newhighP or bottomP:
                    if newlowM:
                        # 2018-01-05 CRESBLD
                        sig, state = -sval, -6
                    elif newhighM:
                        # 2011-11-01 PADINI newhighV
                        sig, state = sval, 7
                    elif bottomM:
                        # 2018-02-05 CRESBLD
                        sig, state = -sval, 7
                ...
                if bottomM and bottomP:
                    ...
                    if topM or plistP[-1] < 0:
                        ...
                        if max(plistM[-3:]) > 10:
                            ...
                            # 2011-10-10 KLSE
                            sig, state = sval, 35
                elif plenV > 1 and plistV[-2] < 0 and nlistP[-1] > 0:  # vvalP == 2 and pvalN == 1:
                    if isprev3bottomM():
                        if isprev3topM:
                            if isprev3bottomP():
                                # 2012-04-16 KLSE
        ...
        if newhighV:
            if newlowC:
                ...
                if newhighC:
                    if newhighP:
                        ...
                        if prevtopV:
                            if plistM[-1] == min(plistM[-3:]) and nlistM[-1] == max(nlistM[-3:]):
                                if pvalley and nlistP[-1] < nlistP[-2]:
                                    # 2017-05-02 DUFU bottomV
                                    sig, state = -sval, -3
                elif bottomM and bottomP:
                    if not (newhighM or newhighP):
                        # 2014-11-24 DUFU
                        sig, state = -sval, 5
            else:
                if newlowM:
                    if newlowP:
                        if prevbottomM:
                            if plistP[-1] == min(plistP[-3:]):
                                # 2017-12-07 DUFU plistP[-1] < 0
                                sig, state = -sval, 10
                elif bottomM:
                    if newlowP:
                        if nlistM[-1] < 5:
                            if plistP[-1] == min(plistP[-3:]):
                                # 2017-12-20 DUFU plistP[-1] < 0
                                sig, state = sval, 20
                elif newhighM:
                    if newhighP:
                        if isprev3bottomM():
            ...
            if bottomC:
                ...
                if topP:
                    if prevbottomM:
                        if nlistM[-1] > 5:
                            if newlowP:
                                ...
                                # 2015-03-16 DUFU
                                # 2016-02-03 ARANK topV
                                sig, state = sval, 12
                elif newlowM:
                    if prevbottomM:
                        if isprev3topP() and plistP[-1] < plistP[-2]:
                            # 2015-05-05 DUFU
                            sig, state = -sval, 20
                elif bottomM:
                    if isprev3topP() and plistP[-1] < plistP[-2]:
                    ...
                    if isprev3bottomM:
                        if isprev3bottomP:
                            if plistV[-1] < 0:
                                if newlowP:
                                    # 2016-11-03 DUFU (6 months bull run)
                                    sig, state = sval, 50
                                else:
                                    # 2016-11-01 DUFU
                                    # 2016-11-11 DUFU
                    ...
                    if nlistM[-2] == min(nlistM):
                        if nlistP[-2] == min(nlistP):
                            if nlistP[-1] > 0:
                                # 2016-10-04 DUFU
                                sig, state = sval, 55
            elif newhighC:
                ...
                if newlowM:
                    if newlowP:
                        if prevbottomP:
                            if mpeak and nlistM[-1] == min(nlistM[-3:]):
                                if ppeak and nlistP[-1] == min(nlistP[-3:]):
                                    # 2017-10-03 DUFU mia==30 (retrace)
                                    sig, state = -sval, 70
                elif topM:
                    if nlistM[-2] == min(nlistM):
                        if nlistP[-2] == min(nlistP):
            ...
            if newhighC:
                if newhighP:
                    if bottomM and not newhighM:
                        # 2015-02-04 DUFU
                        sig, state = -sval, 1
            elif bottomM:
                if prevtopP:
                    if pvalley and nlistP[-1] == min(nlistP[-3:]):
                        if nlistV[-1] == max(nlistV[-3:]):
                            # 2016-04-07 DUFU
                            sig, state = sval, 1
        elif bottomV:
            sval += 5
            ...
            if prevbottomV:
                if newhighM:
                    if isprev3bottomM():
                        if ppeak and plistP[-2] == max(plistP):
                            # 2016-07-12 DUFU
                            sig, state = -sval, 1
                elif plenV > 1 and plistV[-2] == max(plistV):
                    if plenP > 1 and plistP[-2] == max(plistP):
                        if nlenP > 2 and nlistP[-2] == max(nlistP[-3:]):
                            if mpeak and ppeak:
                                # 2017-06-05 DUFU
                                sig, state = -sval, 2
                elif plistV[-1] < 0:
                    if nlistM[-1] < 5 and nlistP[-1] < 0:
                        if bottomM and bottomP:
                            ...
                            # 2015-08-17 DUFU (4 months bull run)
                            sig, state = sval, 1
            elif plenV > 1 and plistV[-2] == max(plistV):
                if newlowP:
                    if topP:
                        if prevbottomM:
                            if nlistM[-1] > 5:
                                # 2015-03-02 DUFU (may not need this)
                                sig, state = sval, 10
                elif newlowM:
                    if prevbottomM:
                        if prevtopP:
                    ...
                    if plistM[-2] == max(plistM):
                        # 2018-12-03 DUFU
                        sig, state = -sval, 15
            elif nlenV > 1 and nlistV[-2] == min(nlistV):
                if plistV[-1] < 0:
                    if nlistM[-2] == min(nlistM) and nlistM[-1] > 5:
                        if nlistP[-2] == min(nlistP):
                            if prevtopC:
                                # 2015-09-02 DUFU
                                sig, state = sval, 20
                            ...
                            if prevtopC and newhighC:
                                if newhighM:
                                    if newhighP:
                                        # 2015-12-11 DUFU
                                        sig, state = sval, 30
            elif plenV > 2 and plistV[-1] == max(plistV[-3:]):
                if bottomM:
                    if bottomP:
                        if nlistM[-1] < 5 and nlistP[-1] < 0:
                            if plistM[-1] == max(plistM[-3:]):
    ...
    def evalVol(sval):
        def evalHighV():
                        ...
                        if newhighM:
                            ...
                            if nlistM[-1] >= 5:
                                if max(plistM[-3:]) >= 10:
                                    ...
                                    if nlistM[-1] == min(nlistM[-3:]):
                                        if plistP[-1] == min(plistP[-3:]):
                                            if plistP[-1] < 0 or newlowP:
                                                # 2012-12-04 PADINI plistP[-1]<0
                                                sig, state = -sval, 13
                                            else:
                                                if lastP < 0:
                                                    # 2018-03-06 PADINI
                                ...
                                if plistP[-1] == min(plistP[-3:]):
                                    if nlistP[-1] < 0:
                                        if plistP[-1] < 0:
                                            if newlowP:
                                                # 2017-12-07 DUFU plistP[-1] < 0, prevbottomM, newlowM
                                                # 2017-12-20 DUFU plistP[-1] < 0
                                                sig, state = -sval, 18
                                            else:
                                                # 2016-04-07 PENTA
                ...
                if not (sig and state):
                    if max(plistM[-3:]) >= 10:
                        if nlistM[-1] >= 5:
                            if nlistP[-1] == max(nlistP[-3:]):
                                if nlistP[-1] < 0:
                                    # 2016-05-09 ARANK newhighM, prevtopP
                                    sig, state = sval, 40
                                elif bottomM:
                                    if plistM[-1] < 10:
                                        # 2017-08-02 CARIMIN
            ...
            def evalHighTopP(sval):
                ...
                if newhighM or topM:
                    if isprev3bottomM():
                        ...
                        if bottomP or nlistP[-1] < 0:
                            if max(plistM[-3:]) >= 10 or \
                                    plistP[-1] == min(plistP[-3:]) or plistP[-1] <= 0:
                                # 2015-11-03 ARANK
                                sig, state = sval, 5
                            elif bottomP or prevtopM:
                                # 2019-03-01 FAVCO
                                sig, state = -sval, 5
                if not (sig and state):
                    if bottomP:
                        ...
                        if nlistM[-1] == max(nlistM[-3:]):
                            ...
                            if plistM[-1] == min(plistM[-3:]):
                                if plistP[-1] < plistP[-2]:
                                    if nlistP[-1] < nlistP[-2]:
                                        if nlistP[-1] < 0:
                                            # 2014-04-22 ARANK bottomP intercepted
                                            sig, state = -sval, 17
                                        else:
                                            sig, state = -sval, 996
                            elif min(nlistP[-3:]) >= 0:
                ...
                if not (sig and state):
                    if max(plistM[-3:]) >= 10:
                        if min(nlistM[-3:]) >= 5:
                            if nlistP[-1] < 0:
                                if plistM[-1] == min(plistM[-3:]):
                                    if ppeak or plistP[-1] == min(plistP[-3:]):
                                        # 2011-08-10 CARLSBG
                                        sig, state = -sval, 9
                                    elif newhighP:
                                        # 2013-06-04 ARANK
                                        ...
                                        sig, state = -sval, 997
                                elif firstP < min(nlistP):
                                    # 2014-04-04 ARANK chartdays=300
                                    sig, state = sval, 10
                                elif plistP[-2] >= 0:
            ...
            def evalNonMP(sval):
                sig, state = 0, 0
                if mpeak and plistM[-1] == max(plistM[-3:]):
                    if nlistM[-1] > nlistM[-2]:
                        if min(nlistM[-3:]) >= 5:
                            if nlistP[-1] == max(nlistP[-3:]):
                                if plistP[-1] < plistP[-2]:
                                ...
                                if plistM[-1] >= 10:
                                    if prevtopM:
                                        if plistP[-1] < plistP[-2]:
                                            if nlistP[-1] == max(nlistP[-3:]):
                                                if nlistP[-1] >= 0:
                                                    # 2012-10-29 ARANK newhighC, prevtopV
                                                    sig, state = sval, -20
                                                else:
                                                    # 2009-06-17 PADINI
                                                    sig, state = sval, 20
            ...
            sig, state = 0, 0
            if newlowM or bottomM:
                sig, state = evalLowBottomM(sval + 1)
            if not sig or state == 0:
                if newhighP or topP:
                    sig, state = evalHighTopP(sval + 2)
            if not sig or state == 0:
                if newlowP or bottomP:
                    sig, state = evalLowBottomP(sval + 3)
            if not sig or state == 0:
                if newhighM or topM:
                    sig, state = evalHighTopM(sval + 4)
            if not sig or state == 0:
                if prevtopP:
                    sig, state = evalPrevtopP(sval + 5)
            if not sig or state == 0:
                if prevtopM:
                    sig, state = evalPrevtopM(sval + 6)
            if not sig or state == 0:
                if prevbottomP:
                    sig, state = evalPrevbottomP(sval + 7)
            if not sig or state == 0:
                if prevbottomM:
                    sig, state = evalPrevbottomM(sval + 8)
            if not sig or state == 0:
                sig, state = evalNonMP(sval)
            return sig, state
        ...
        def evalLowV():
                        ...
                        if topP:
                            if prevtopM:
                                if nlistM[-1] == min(nlistM[-3:]):
                                    if nlistM[-1] >= 5:
                                        if nlistP[-1] < 0:
                                            # 2012-05-14 ARANK topV, prevbottomV
                                            sig, state = sval, 50
                                        else:
                                            sig, state = -sval, 995
                if not (sig and state):
                            ...
                            if nlistP[-1] >= 0:
                                if plistM[-1] < 10:
                                    if bottomM:
                                        ...
                                        # 2012-08-01 CRESBLD
                                        sig, state = -sval, 70
                                elif plistM[-1] == max(plistM[-3:]) or newlowM:
                                    # 2010-04-01 PADINI
                                    sig, state = -sval, 71
                                elif nlistP[-1] == max(nlistP[-3:]):
                                    # 2014-07-01 PENTA
                        ...
                        if nlistP[-1] == max(nlistP[-3:]):
                            if max(plistM[-3:]) < 10:
                                if topM:
                                    ...
                                    # 2012-01-03 KLSE
                                    # 2012-04-06 PADINI topC, topV
                                    sig, state = sval, 73
                            else:
                                if min(plistM[-3:]) >= 10 or nlistM[-1] >= 10:
                                    # 2014-06-09 DUFU bottomC
                                    sig, state = sval, 74
                                elif plistM[-1] >= 10:
                                    # 2010-04-05 PADINI
                                    sig, state = -sval, 74
                        ...
                        if topM:
                            if plistM[-1] < 10:
                                if nlistP[-1] >= 0:
                                    ...
                                    sig, state = sval, -999
                        elif mpeak and (nlistM[-1] == min(nlistM[-3:]) or
                                        max(plistM[-3:]) >= 10):
                            if nlistP[-1] < 0:
                                # 2012-11-12 CRESBLD
                                # 2017-10-03 DUFU mia==30 (retrace)
                        ...
                        if prevtopP:
                            if plistM[-1] >= 10:
                                if min(nlistM[-3:]) >= 5:
                                    if nlistP[-1] == max(nlistP[-3:]):
                                        if nlistP[-1] >= 0:
                                            # 2019-02-28 DANCO only when chartdays = 300
                                            sig, state = sval, 10
                                        else:
                                            sig, state = -sval, 995
                        elif plistM[-1] >= 10:
                ...
                if not (sig and state):
                    if max(plistM[-3:]) < 10:
                        if nlistM[-2] == min(nlistM):
                            ...
                            if min(nlistP[-3:]) >= 0:
                                # 2013-12-02 CRESBLD newlowP
                                sig, state = sval, 12
                            elif nlistM[-1] < 5:
                                # 2011-11-14 KLSE bottomM, bottomP, newhighM, newhighP
                                sig, state = sval, -12
                            else:
                                # 2018-06-04 ARANK
                                sig, state = -sval, 12
                    ...
                    if prevtopP:
                        if plistM[-2] == max(plistM):
                            if nlistM[-1] == min(nlistM[-3:]):
                                if nlistM[-1] >= 5:
                                    if nlistP[-1] < 0:
                                        # 2012-06-15 ARANK topV, prevbottomV
                                        sig, state = sval, 5
                                    else:
                                        sig, state = -sval, 998
                if not (sig and state):
                    ...
                    if min(nlistP[-3:]) >= 0:
                        ...
                        if max(plistM[-3:]) < 10:
                            if isprev3topP():
                                ...
                                # 2017-01-04 PADINI topV, plistP[-1]<0
                                sig, state = sval, -20
                            elif prevtopP and prevbottomM:
                                # 2012-11-02 PADINI
                                sig, state = -sval, 20
                            elif nlistP[-1] < 0:
                                if nlistM[-1] == max(nlistM[-3:]):
                    ...
                    if isprev3bottomP:
                        if plistV[-1] < 0:
                            if newlowP:
                                ...
                                sig, state = sval, 999
                        elif plistP[-1] < plistP[-2]:
                            if plistM[-1] >= 10:
                                if nlistM[-1] >= 5:
                                    if nlistM[-1] == max(nlistM[-3:]):
                                        # 2015-06-10 ARANK newlowC
                                        sig, state = -sval, 5
                                    else:
                                        # 2014-01-02 ARANK topC
                                        sig, state = sval, 5
                return sig, state

            def evalPrevbottomM(sval):
                sig, state = 0, 0
                if plistM[-1] == min(plistM[-3:]) and plistM[-1] < 5 and \
                        plistP[-1] == min(plistP[-3:]):
                    # 2008-12-01 KLSE
                    sig, state = -sval, 1
                elif prevbottomP:
                    if plistM[-1] == min(plistM[-3:]):
            ...
            def evalPrevbottomP(sval):
                ...
                if nlistP[-1] < 0:
                    if min(nlistM[-3:]) >= 5:
                        if mpeak and plistM[-1] < plistM[-2]:
                            if max(plistM[-3:]) >= 10:
                                # 2012-11-01 CRESBLD
                                sig, state = -sval, 1
                            elif plistP[-1] == min(plistP[-3:]):
            ...
            def evalPrevtopP(sval):
                ...
                if nlistP[-1] >= 0:
                    if nlistM[-1] >= 5:
                        if mpeak and plistM[-1] < plistM[-2]:
                            if max(plistM[-3:]) < 10:
                                if nlistP[-2] == max(nlistP[-3:]):
                                    # 2013-08-02 CRESBLD plistV[-1] < 0
                                    sig, state = sval, 1
                                elif nlistP[-2] < 0:
                                    # 2016-09-01 CRESBLD
                                    sig, state = -sval, 1
            ...
            def evalNonMP(sval):
                ...
                if max(plistM[-3:]) >= 10:
                    if min(nlistM[-3:]) >= 5:
                        if plenP > 2 and plistP[-1] == min(plistP[-3:]):
                            if isprev3bottomP():
                                if nlistP[-1] < 0:
                                    if bottomP:
                                        # 2011-09-13 CARLSBG
                    ...
                    if nlistP[-1] > 0:
                        ...
                        if nlistM[-1] >= 5:
                            if plistM[-1] > plistM[-2]:
                                if plistP[-1] == max(plistP[-3:]) or topM:
                                    # 2014-05-02 CARLSBG
                                    sig, state = -sval, 15
                                else:
                                    # 2015-08-03 PADINI
                                ...
                                # 2016-05-03 PENTA bottomP, newhighM
                                sig, state = sval, 16
                            else:
                                # 2014-03-17 CARLSBG
                                sig, state = -sval, 16
                        ...
                        if max(nlistP[-3:]) < 0:
                            ...
                            if nlistM[-1] == max(nlistM[-3:]):
                                if nlistP[-1] == min(nlistP[-3:]):
                                    ...
                                    # if nlistP[-2] == min(nlistP[-3:]):
                                    # 2018-01-03 PADINI
                                    sig, state = -sval, 37
                            elif nlistM[-1] == min(nlistM[-3:]):
                                if min(nlistP[-3:]) >= 0:
                                    if mvalley and pvalley:
                                        # 2013-10-03 CRESBLD
                                        sig, state = sval, 45
                                    else:
                                        sig, state = sval, -993
            ...
            if not sig or state == 0:
                if newhighM or topM:
                    sig, state = evalHighTopM(sval + 3)
            if not sig or state == 0:
                if newlowP or bottomP:
                    sig, state = evalLowBottomP(sval + 4)
            if not sig or state == 0:
                if prevtopM:
                    sig, state = evalPrevtopM(sval + 5)
            if not sig or state == 0:
                if prevbottomM:
                    sig, state = evalPrevbottomM(sval + 6)
            if not sig or state == 0:
                if prevbottomP:
                    sig, state = evalPrevbottomP(sval + 7)
            if not sig or state == 0:
                if prevtopP:
                    sig, state = evalPrevtopP(sval + 8)
            if not sig or state == 0:
                sig, state = evalNonMP(sval)
            return sig, state
        ...
        def evalTopV():
                            ...
                            if nlistP[-1] >= 0:
                                if nlistM[-1] >= 5:
                                    if plistM[-1] < 10:
                                        ...
                                        sig, state = sval, 998
                if not (sig and state):
                    if newhighM or topM or prevtopM:
                        if newhighP or topP or prevtopP:
                            if plistM[-1] < 10:
                                if nlistM[-1] >= 5:
                                    ...
                                    if plistP[-1] == min(plistP[-3:]):
                                        # 2013-03-14 CARLSBG newhighM, newhighP
                                        sig, state = sval, 13
                                    else:
                                        # 2014-04-08 PADINI topM, topP
                                        sig, state = -sval, 13
                                elif nlistP[-1] >= 0:
                                    if newlowP:
                                        if nlistM[-1] < 5:
                    ...
                    if plistM[-1] < plistM[-2]:
                        if mpeak and nlistM[-1] > nlistM[-2]:
                            ...
                            if nlistP[-1] == max(nlistP[-3:]):
                                ...
                                sig, state = -sval, 994
                        elif plistP[-1] == max(plistP[-3:]) and \
                                nlistP[-1] == max(nlistP[-3:]):
                            if min(nlistM[-3:]) >= 5:
                                if plistM[-1] < 10:
                                    if prevtopP:
                                    ...
                                    sig, state = sval, 27
                    elif nlistM[-1] == max(nlistM[-3:]) or nlistM[-2] == max(nlistM[-3:]):
                        if nlistP[-1] == max(nlistP[-3:]):
                            if nlistP[-1] >= 0:
                                if nlistM[-1] >= 5:
                                    if plistM[-1] < 10:
                        ...
                        if prevtopM:
                            if plistM[-1] >= 10:
                                if min(nlistM[-3:]) >= 5:
                                    if nlistP[-1] == max(nlistP[-3:]):
                                        if nlistP[-1] >= 0:
                                            # 2019-01-25 DANCO prevtopV, newlowV
                                            sig, state = sval, 5
                                        else:
                                            sig, state = -sval, 999
                        elif pvalley and nlistP[-1] == min(nlistP[-3:]):
                            if nlistV[-1] == max(nlistV[-3:]):
                                # 2016-04-15 DUFU
                                sig, state = sval, 10
                            else:
                            ...
                            if nlistP[-1] == max(nlistP[-3:]):
                                if nlistP[-1] >= 0:
                                    if nlistM[-1] == max(nlistM[-3:]):
                                        ...
                                        # 2019-01-25 DANCO prevtopV
                                        # 2019-02-28 DANCO prevtopV, newlowV
                                        sig, state = sval, 1
                                else:
                                    sig, state = -sval, 999
                return sig, state
                ...
                if plistM[-1] == min(plistM[-3:]) and plistM[-1] < 5 and \
                    ...
                    if min(nlistM[-3:]) >= 5:
                        ...
                        if isprev3topP():
                            if isprev3bottomP():
                                if nlistP[-1] < 0:
                                    if mpeak and plistM[-1] == min(plistM[-3:]):
                                        if plistM[-1] == min(plistM[-3:]):
                                            # 2014-01-22 PADINI
                                            sig, state = sval, -20
                                        else:
                                        ...
                                        if plistP[-1] < 0:
                                            # 2016-08-01 PENTA bottomP, prevtopM
                                            sig, state = sval, 20
                                        else:
                                            sig, state = sval, -999
                                    else:
                            ...
                            if isprev3bottomP():
                                if nlistP[-1] < 0:
                                    if mpeak and plistM[-1] == min(plistM[-3:]):
                                        ...
                                        if plistP[-1] == min(plistP[-3:]) or newhighM:
                                            # 2015-02-24 PENTA
                                            # 2017-03-09 PADINI bottomP, plistP[-1]<0
                                            # 2018-06-04 PADINI
                                            sig, state = sval, 35
                                        else:
                                            # 2015-09-03 CRESBLD
            ...
            if not sig or state == 0:
                if newhighM or topM:
                    sig, state = evalHighTopM(sval + 2)
            if not sig or state == 0:
                if newlowP or bottomP:
                    sig, state = evalLowBottomP(sval + 3)
            if not sig or state == 0:
                if newlowM or bottomM:
                    sig, state = evalLowBottomM(sval + 4)
            if not sig or state == 0:
                if prevtopM:
                    sig, state = evalPrevtopM(sval + 5)
            if not sig or state == 0:
        ...
        def evalBottomV():
            def evalHighTopM(sval):
                ...
                if bottomM:
                    if bottomP:
                        if nlistM[-1] >= 5:
                            if nlistP[-1] < 0:
                                if prevtopP or prevtopM:
                                    # 2010-05-19 PADINI
                                    # 2012-07-31 ARANK newhighM
                                    sig, state = sval, 1
                                else:
                                    sig, state = -sval, 999
                if not (sig and state):
            ...
            def evalLowBottomP(sval):
                ...
                if bottomM or prevbottomM:
                    if nlistM[-1] < 5 and nlistP[-1] < 0:
                        if max(plistM[-3:]) >= 10:
                            if prevtopP or prevbottomM or plistM[-1] >= 10:
                                # 2011-05-26 PADINI
                                sig, state = -sval, 1
                            else:
                                if prevtopC:
                            ...
                            # 2014-11-04 ARANK
                            sig, state = -sval, 2
                    elif newhighM or prevtopP or (prevtopM and plistM[-1] < 10):
                        if nlistP[-1] < 0:
                            if max(plistM[-3:]) < 10:
                                # 2012-07-31 ARANK
                                sig, state = sval, 10
                        ...
                        if min(nlistM[-3:]) >= 5:
                            if min(nlistP[-3:]) >= 0:
                                ...
                                if bottomP or prevtopP:
                                    if plistM[-1] == min(plistM[-3:]):
                                        if prevbottomM:
                                            # 2015-03-19 CRESBLD plistV[-1] < 0
                                            sig, state = -sval, 30
                                        else:
                                            # 2015-01-09 PENTA
                                            sig, state = sval, 30
                                    else:
                                        # 2011-07-15 CARLSBG prevtopP
                                        sig, state = -sval, 31
                                elif topP or bottomM:
                                    if plistP[-1] >= 0:
                                        # 2014-06-13 ARANK
                                        sig, state = sval, 31
                                    else:
        ...
        def evalPrevtopV():
            ...
            if newhighP or topP:
                if bottomM and not newhighM:
                    if plistM[-1] == min(plistM[-3:]) or min(nlistP[-3:]) >= 0:
                        if nlistM[-1] >= 5:
                            # 2015-02-04 DUFU
                            sig, state = -sval, 1
                        else:
            ...
            if not (sig and state):
                if newhighM or topM:
                    if bottomM:
                        if plenM > 1 and plistM[-1] < plistM[-2]:
                            if plistM[-2] < 10:
                                if nlistM[-1] < 5:
                                    if nlistP[-1] >= 0:
                                        # 2013-05-03 CARLSBG
                                ...
                                if nlistP[-1] == max(nlistP[-3:]):
                                    if nlistP[-1] >= 0:
                                        if plistP[-1] == min(plistP[-3:]):
                                            ...
                                            # 2009-12-14 PADINI newhighM
                                            sig, state = sval, 9
                                        else:
                                            # 2014-02-13 DUFU newlowC, topM
                                            # 2018-08-29 CARIMIN newlowC, topM
                                            sig, state = sval, -9
                        elif nlistM[-1] >= 5:
                            if nlistP[-1] == max(nlistP[-3:]):
                                if nlistP[-1] >= 0:
                            ...
                            if plistM[-1] < 10:
                                if nlistM[-1] >= 5:
                                    if min(nlistP[-3:]) >= 0:
                                        # 2012-07-02 CRESBLD
                                        sig, state = -sval, 15
                                    elif newhighP or nlistP[-1] > min(plistP[-3:]):
                                        # 2012-03-01 CARLSBG
                                        sig, state = sval, 15
                                    else:
                                        # 2019-02-14 DANCO
            ...
            if not (sig and state):
                if newlowM:
                    if newlowP:
                        if nlistM[-1] >= 5:
                            if plenM > 2 and plistM[-1] == min(plistM[-3:]):
                                if isprev3topP() and isprev3bottomP():
                                    if nlistP[-1] < 0:
                                        # 2014-02-05 PADINI
                                        sig, state = sval, 30
        ...
        def evalPrevbottomV():
                ...
                if isprev3bottomM():
                    if ppeak and plistP[-2] == max(plistP):
                        ...
                        if max(plistM[-3:]) < 10:
                            ...
                            if nlistP[-1] == max(nlistP[-3:]):
                                # 2014-02-25 DUFU prevtopP
                                sig, state = sval, -5
                            elif nlistP[-1] == min(nlistP[-3:]):
                                # 2016-07-04 PENTA prevbottomP
                                sig, state = sval, 5
                    elif nlistM[-1] == max(nlistM[-3:]):
                        if nlistP[-1] == max(nlistP[-3:]):
                            sig, state = sval, 7
            ...
            def evalLowBottomM(sval):
                ...
                if prevbottomM:
                    if max(plistM[-3:]) >= 10:
                        if nlistM[-1] >= 5:
                            if plenP > 2 and plistP[-2] == max(plistP[-3:]):
                                if nlistP[-1] == max(nlistP[-3:]):
                                    if nlistP[-1] < 0:
                                        # 2016-04-01 ARANK
                                        sig, state = sval, 1
            ...
            def evalNonMP(sval):
                sig, state = 0, 0
                if plenV > 1 and plistV[-2] == max(plistV):
                    if plenP > 1 and plistP[-2] == max(plistP):
                        if nlenP > 2 and nlistP[-2] == max(nlistP[-3:]):
                            if mpeak and ppeak:
                                if nlistP[-1] < 0:
                                    if nlistM[-1] == min(nlistM[-3:]):
                                        # 2010-06-02 PADINI newlowM
                                        # 2019-03-25 PENTA
                                        sig, state = sval, 1
                                elif nlistP[-1] == min(nlistP[-3:]):
                                    # 2009-12-30 CARLSBG
                            ...
                            if bottomM:
                                if nlistM[-1] >= 5:
                                    if nlistP[-1] >= 0:
                                        if plenM > 2 and max(plistM[-3:]) < 10:
            ...
            if not (sig and state):
                if prevbottomM:
                    sig, state = evalPrevbottomM(sval + 3)
            if not (sig and state):
                if prevtopM:
                    if max(plistM[-3:]) < 10:
                        if min(nlistM[-3:]) >= 5:
        ...
        def evalOtherVs():
            def evalP2Max(sval):
                ...
                if topP:
                    if newlowP:
                        if prevbottomM:
                            if nlistM[-1] >= 5:
                                if plenM > 2 and max(plistM[-3:]) < 10 or \
                                        nlenP > 2 and max(nlistP[-3:]) <= 0:
                                    sig, state = -sval, 999
                                else:
                                    if vpeak and nlistV[-1] >= 0:
                                        # 2015-03-02 DUFU topC (may not need this)
                                        sig, state = sval, 1
                                    else:
                                        # 2012-09-25 CRESBLD
                                        sig, state = -sval, 2
                            ...
                            if nlistM[-1] < 5:
                                if max(nlistP[-3:]) < 0:
                                    if plistM[-1] == min(plistM[-3:]):
                                        ...
                                        sig, state = -sval, 997
                if not (sig and state):
                    if prevtopP or (nlistP[-2] >= 0 and plistP[-1] > plistP[-2]):
                        if plenM > 1 and plistM[-2] == max(plistM):
                            if bottomP or nlistP[-1] < 0 or newhighM or newhighP:
                                if min(nlistM[-3:]) >= 5:
                                    if nlistP[-1] == max(nlistP[-3:]):
                                        if nlistP[-1] > 0:
                                            # 2012-03-05 CARLSBG
                                ...
                                if max(nlistP[:-1]) < 0:
                                    if tripleM == 7 and tripleP == 7:
                                        if plistM[-3] > 8 and nlistM[-1] > 8:
                                            # 2016-02-11 PADINI
                                            sig, state = sval, -20
                                        else:
                                            # 2018-12-03 DUFU
                                ...
                                if nlistP[-1] >= 0:
                                    if topM:
                                        if prevtopP:
                                            # 2013-06-04 CARLSBG
                                            sig, state = -sval, 29
                                        elif ppeak and plistP[-2] == max(plistP):
                                            # 2013-06-12 CARLSBG
                                            sig, state = -sval, 30
                                        else:
                                            # 2019-04-08 PENTA
                                    ...
                                    if plistP[-1] < 0:
                                        # 2016-03-16 PENTA newlowM, newlowP
                                        sig, state = sval, -31
                                    else:
                                        # 2014-12-04 ARANK
                                        sig, state = -sval, 31
                        ...
                        if nlistM[-1] >= 5:
                            if nlistP[-1] >= 0:
                                if nlistM[-2] == max(nlistM[-3:]):
                                    if nlistP[-2] == max(nlistP[-3:]):
                                        if plistP[-1] == min(plistP[-3:]):
                                            if min(nlistP[-3:]) >= 0 or min(nlistM[-3:]) >= 5:
                                                # 2014-08-08 KLSE
                                                sig, state = -sval, -50
                                            else:
                                                # 2013-09-03 CRESBLD
                    ...
                    if nlistM[-2] == min(nlistM) and nlistM[-1] >= 5:
                        if nlistP[-2] == min(nlistP):
                            if max(plistM[-3:]) < 10:
                                ...
                                # 2015-09-02 DUFU
                                sig, state = sval, 4
                    elif nlenM > 2 and nlistM[-3] == min(nlistM) and nlistM[-1] == max(nlistM[-3:]):
                        if nlenP > 2 and nlistP[-3] == min(nlistP) and nlistP[-1] == max(nlistP[-3:]):
                            if prevtopC and newhighC:
                                if newhighM:
                                    # 2015-12-03 DUFU
                                    sig, state = sval, 7
            ...
            def evalMP(sval):
                ...
                if isprev3topV():
                    ...
                    if newhighP:
                        ...
                        if pvalley and nlistP[-1] < 0:
                            if mvalley and nlistM[-1] == max(nlistM[-3:]):
                                if nlistM[-1] >= 5:
                                    if plistM[-1] < 10:
                                        if nlistM[-2] == min(nlistM) and \
                                                nlistP[-2] == min(nlistP):
                                            # 2013-03-01 CRESBLD
                                            # 2018-06-06 PENTA
                                            sig, state = sval, 20
            ...
            def evalNonMP(sval):
                sig, state = 0, 0
                if plenV > 2 and plistV[-1] == max(plistV[-3:]):
                    if bottomM:
                        if bottomP:
                            if nlistM[-1] < 5 and nlistP[-1] < 0:
                                if plistM[-1] == max(plistM[-3:]):
                                    if plistP[-1] == min(plistP[-3:]):
                                        ...
                                        sig, state = sval, 999
                if not (sig and state):
                    if plenV > 2 and (plistV[-1] == min(plistV[-3:]) or
                                      plistV[-2] == min(plistV[-3:])):
                        if pvalley and plistV[-1] < 0 or plistV[-2] < 0:
                            if prevtopM:
                                if max(plistM[-3:]) < 10:
                                    if nlistM[-1] >= 5:
                                        if plistP[-1] == min(plistP[-3:]):
            ...
            sig, state = 0, 0
            if plenV > 1 and plistV[-2] == max(plistV):
                sig, state = evalP2Max(sval + 1)
            if not (sig and state):
                if nlenV > 1 and nlistV[-2] == min(nlistV):
                    sig, state = evalN2Min(sval + 2)
            if not sig:
                sig, state = evalMP(sval + 3)
            if not (sig and state):
                sig, state = evalNonMP(sval)
            return sig, state
        ...
        if not (sig and state):
            if bottomV:
                ...
                if not mia:
                    mia = sval
                sig, state = evalBottomV()
        if not (sig and state):
            if prevtopV:
                sval = 50
                if not mia:
                    mia = sval
                sig, state = evalPrevtopV()
        if not (sig and state):
            if prevbottomV:
                sval = 60
                if not mia:
                    mia = sval
                sig, state = evalPrevbottomV()
        if not (sig and state):
                sval = 70
                if not mia:
                    mia = sval
                sig, state = evalOtherVs()

        if not (sig and state):
            if posV in [0, 4] or bottomV or topV or prevbottomV or prevtopV:
                pass
            elif posM in [0, 4] or topM or bottomM or prevtopM or prevbottomM:
                mia = 80
            elif posP in [0, 4] or topP or bottomP or prevtopP or prevbottomP:
                mia = 90
            if mia:
                sig = mia
        return sig, state
    ...
    if plistM is None or plistP is None or nlistM is None or nlistP is None:
        nosignal = True
    elif (len(nlistP) < 2 or len(plistP) < 2 or
          len(nlistM) < 2 or len(plistM) < 2) and busdays < 345:
        ...
        if ssig:
            ...
            if not ssig or sstate > 900:
                if newhighC or (firstC == maxC and lastC > highbar):
                    ...
                    pass
                        ...
                        (topC and nlenC < 3 and plenC > 1 and lastC < plistC[-2]) or \
            ...
            if not ssig or sstate > 900:
                if (bottomC or narrowC == 9) and lastC > highbar or \
                        (plenC > 1 and nlenC > 1 and nlistC[-2] == min(nlistC) and plistC[-1] < midbar) or \
    ...
    def plot(scode, showchart):
        ...
        if args['--plot']:
            ...
            if showchart:
                ...
                if len(signals):
                    # outname = S.DATA_DIR + "mpv/synopsis/" + counter
